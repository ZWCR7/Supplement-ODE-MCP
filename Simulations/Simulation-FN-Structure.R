...
#Systematic Structural Change with Normal Errors
#############################################################################################################################################################################################
rm(list = ls())
pars = c("a", "b", "c")
...
for (set_s in 1:length(gammav))
{
  ...
  Simu = function(s)
  {
    ...
    detect_res_ER = SNot(bi_stat, thres_beta = NULL, thres_GLR = NULL, q.max = 20, method = 'upper.bound', Statistic = 'ER', gap = m/2)
    ...
    detect_res_GLR = SNot(bi_stat, thres_beta = NULL, thres_GLR = NULL, q.max = 20, method = 'upper.bound', Statistic = 'GLR', gap = m/2)
  ...
  save(list = c('CP_RES', 'true_cp'), file = paste0('Results_FN/Mops-FN-Structural_n = ', n, '_K = ', K, '_noise sigma_N', set_s, '.RData'))
}

############################################################################################################################################################################################################################

#Systematic Structural Change with t Errors
#############################################################################################################################################################################################
rm(list = ls())
pars = c("a", "b", "c")
...
for (set_s in 1:length(gammav))
{
  ...
  save(list = c('CP_RES', 'true_cp'), file = paste0('Results_FN/Mops-FN-Structural_n = ', n, '_K = ', K, '_noise sigma_T', set_s, '.RData'))
}
############################################################################################################################################################################################################################
